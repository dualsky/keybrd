...
{ 
    ...
    public:
        ...
        //      c_RowPort_PCA9655E rowPort1_L(port1_L, 7, 3, 1<<0 | 1<<1 );
        c_RowPort_PCA9655E(c_IOExpanderPort& ep, const uint8_t config,
                const uint8_t output, const uint8_t PINS):
                port(ep), configuration(config), output(output), c_RowPort(PINS) {}
